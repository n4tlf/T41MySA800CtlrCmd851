...
#ifdef  TEENSY4
//  First we define the output signal pins to the SA-800 drive
#define STEP  31         // SA800 #36  step the head one track active low
#define DIR   32         // SA800 #34  step direction (out=high, in=low)
#define DS0   26         // SA800 #26 Drive Select 0  (port PC0)
#define DS1   27         // SA800 #28 Drive Select 1  (port PC1)
#define DS2   28         // SA800 #30 Drive Select 2  (port PC2)
#define DS3   29         // SA800 #32 Drive Select 3  (port PC3)
#define HEADLD 30        // SA800 #18 head load  (LOW = HEAD LOAD)
#define WRDATA  33       // SA800 #38 Write Data (invert this with 7406)
#define WRGATE  34       // SA800 #40  Write Gate
#define TG43    35       // SA800 #2   Track Greater than 43 (SA-800 not used)
#define SIDESEL 37       // SA851 #14  Side Select (SA851) High=side 0, low=side 1
#define INUSE   36       // SA800 #16  in use signal (not used on SA-800)

// Next comes the input signal pins from the SA-800 drive
#define TRK00   38     // SA800 #42  Head at Track 00 (00=low, not=high) generates INT
#define INDEX   39     // SA800 #20  Index hole pulses  generates INT
#define DREADY  40     // SA800 #22  Drive ready
#define READDTA 41     // SA800 #46  Raw Read Data
#define WRPROT  7      // SA800 #44  Write protect detection
#define CHDISK  6      // SA800 #12  Disk changed (not used on SA-800)
#define SECTOR  5      // SA800 #24  Sector pulses (not used on SA-800)
...
void tftUpdate() {
  tft.fillScreen(ILI9341_BLACK);
      tft.setCursor(30, ROW1);
      tft.print("SA-800 Foxerciser");

  tft.setCursor(10,ROW2);
  tft.print("DR:");
  tft.print(curDrive);
  tft.setCursor(80,ROW2);
  tft.print("Trk:  ");
  tft.setCursor(130,ROW2);
  tft.print(currentTrack);
  tft.setCursor(170,ROW2);
  tft.print("Sec:   ");
  tft.setCursor(230,ROW2);
  tft.print(curSec);
  tft.setCursor(260,ROW2);
  if (noWrite && writeProtect)
    tft.print("B");
  else if (noWrite)
    tft.print("N");
  ...
  tft.print("Side:");
  tft.setCursor(70,ROW3);
  tft.print(!curSide);
  tft.setCursor(120,ROW3);
  tft.print("Head:");
  tft.setCursor(180,ROW3);
  if (headLoaded)
    tft.println("LOADED!  ");
  else
    tft.println("UNLOADED");
  tft.fillRect(10,180,150,220, ILI9341_BLUE);
  tft.fillRect(150,180,290,220, ILI9341_RED);
}
...
//  Timer1.initialize(80000000);
//  Timer1.attachInterrupt(display);
  
  attachInterrupt(digitalPinToInterrupt(TRK00),IsrTrk0,FALLING);
//  attachInterrupt(digitalPinToInterrupt(INDEX),IsrIndex,FALLING);
    ...
    if (inByte == 0);
      ...
      switch (inByte)
      {
        ...
        case 0x0A:    //'\n':              //0x0a:
          ...
          for (indexf = 0; indexf < cmdLen; indexf++)
          {
            cmdBfr[indexf] = '\0';
          }
          indexf = 0;
    ...
    case '2':
      if (!noWrite && !writeProtect)
      {
        if (driveReady)
        {
          ...
          analogWrite(3,0);
          analogWriteFrequency(3,500000);     // 500kHz = both Clock AND Data pulses (FF or Shugart 2F)
          analogWrite(3,64);    // 32 = 220ns, 64 = 500ns pulse width
          digitalWrite(WRGATE, LOW);
